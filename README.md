...
| 型號 | Port 數 | 晶片 | 備註 |
|------|---------|------|------|
| RSHTECH RSH-ST07C | 7 (4 可控) | VIA Labs VL822 | 價格實惠 |
| RSHTECH RSH-A37S | 7 | VIA Labs VL822 | 全 port 可控 |
| Plugable USB3-HUB7BC | 7 | VIA Labs VL813 | 社群推薦 |
| AmazonBasics HU9002V1 | 7 | VIA Labs VL817 | 容易取得 |
...
Stage 1: Setup Wizard (Pre-ADB)
    ...
    │  USB 偵測超時時自動 power cycle 恢復（若配置 usb_power）
    ▼
Stage 2: ADB Bootstrap + Pre-test Setup
    │  等待 ADB 連線 → FBE 解鎖 → WiFi 連線（子系統就緒等待）
...
```
Factory Reset → USB Power Cycle → ADB 等待 → Stage 1-4
```
...
| 測試 | pytest + pytest-mock (255 tests) |
