...
## 內建測試套件 (smoke_basic — 69 項)

| 類別 | 測試數 | 測試項目 |
|------|--------|---------|
| 基礎檢查 (adb) | 32 | 開機完成、螢幕顯示(LLM)、觸控回應、WiFi 連線、網路存取、SIM 卡狀態、相機可用、音效輸出、GPS Provider/定位/衛星訊號、藍牙啟用/Adapter/MAC、NFC、加速度計(存在+數據)、陀螺儀(存在+數據)、光線感測器、磁力計、儲存、記憶體、亮度寫入驗證、自動旋轉、電池、充電、系統崩潰、SELinux、DNS、SurfaceFlinger、Activity Manager |
| 相機功能 | 4 | 後鏡頭拍照、前鏡頭拍照、照片品質驗證(LLM)、錄影功能 |
...
| 電源管理 | 2 | Suspend/Resume + Deep Sleep 驗證（soc_sleep stats）、ADB Reboot 驗證 |
...
| 測試 | pytest + pytest-mock (283 tests) |
