...
- **Pre-ADB 自動化** — 透過 AOA2 USB HID 模擬觸控/鍵盤，支援 Blind Runner（預錄 YAML 步驟播放）和 LLM Vision（Webcam + AI 分析）兩種模式完成 Setup Wizard
...
Stage 1: Setup Wizard (Pre-ADB)
    │  方式 A: Blind Runner — 播放預錄的 YAML 步驟檔（固定座標）
    │  方式 B: LLM Vision — AOA2 HID + Webcam + LLM 自動分析畫面
    │  自動完成語言選擇、WiFi 設定、開發者選項、USB debugging 啟用
    ▼
...
| 測試 | pytest + pytest-mock (194 tests) |
